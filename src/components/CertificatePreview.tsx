...
import { Download, ZoomIn, ZoomOut, ChevronLeft, ChevronRight, Check } from 'lucide-react';
...
export const CertificatePreview: React.FC<CertificatePreviewProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-slate-100/60 border border-slate-200 overflow-hidden">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-2.5 flex items-center justify-between gap-2">
        {/* Recipient Selector */}
        <div className="flex items-center gap-1.5 text-xs text-slate-700">
          <button
            onClick={handlePrev}
            disabled={currentIndex === 0}
            className="p-1 hover:bg-slate-100 disabled:opacity-30 cursor-pointer transition"
            title="Anterior"
          >
            <ChevronLeft className="w-4 h-4" />
          </button>
          <span className="font-medium px-1 text-slate-800">
        ...
        {/* Actions: Zoom & Download */}
        <div className="flex items-center gap-2">
          <div className="flex items-center text-xs text-slate-600">
            <button
              onClick={() => setScale((s) => Math.max(0.45, s - 0.08))}
              className="p-1 hover:bg-slate-100 cursor-pointer transition"
              title="Diminuir"
            >
              <ZoomOut className="w-3.5 h-3.5" />
            </button>
            <span className="px-1 text-[11px] font-mono w-9 text-center">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-medium px-3 py-1.5 disabled:opacity-50 transition cursor-pointer"
          >
            {isExporting ? (
              <>
      ...
      <div className="flex-1 overflow-auto p-4 sm:p-6 flex items-center justify-center relative">
        <div
          style={{ width: `${892 * scale}px`, height: `${621 * scale}px` }}
          className="transition-all duration-100 ease-out shadow-md bg-white"
        >
          {activeSide === 'frente' ? (
