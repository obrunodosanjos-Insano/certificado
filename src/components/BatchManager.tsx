    ...
    <div className="bg-white border border-slate-200 shadow-xs flex flex-col h-full overflow-hidden">
      <div className="p-4 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={downloadExcelTemplate} className="px-3 py-2 border border-emerald-300 bg-emerald-50 text-emerald-800 text-xs font-semibold flex items-center gap-1.5 cursor-pointer">
            <Download className="w-3.5 h-3.5" /> Baixar planilha modelo
          </button>
          <button onClick={() => fileInputRef.current?.click()} className="px-3 py-2 border text-xs font-semibold flex items-center gap-1.5 cursor-pointer">
          ...
          />
          <button onClick={addRecipient} className="px-3 py-2 bg-slate-100 text-xs font-semibold flex items-center gap-1.5 cursor-pointer">
      ...
      <div className="px-4 py-3 bg-slate-50 border-b flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button disabled={isGenerating} onClick={() => run('pdf')} className="px-3 py-2 border bg-white text-xs font-semibold flex items-center gap-1.5 disabled:opacity-50 cursor-pointer">
            <Layers className="w-3.5 h-3.5" /> PDF único
          </button>
          <button disabled={isGenerating} onClick={() => run('zip')} className="px-3 py-2 bg-blue-600 text-white text-xs font-semibold flex items-center gap-1.5 disabled:opacity-50 cursor-pointer">
          ...
          <tbody>
            {recipients.map((r, index) => (
              <tr key={r.id} onClick={() => onSelectIndex(index)} className={`border-b border-slate-100 ${index === currentIndex ? 'bg-blue-50' : 'hover:bg-slate-50'}`}>
                <td className="p-2"><input value={certificateCodeForIndex(index)} readOnly title="Numeração automática" className="w-full border px-2 py-1.5 font-mono bg-slate-50 text-slate-700 cursor-not-allowed" /></td>
                <td className="p-2"><input value={r.name} onChange={(e) => update(r.id, 'name', e.target.value.toUpperCase())} className="w-full border px-2 py-1.5 font-semibold uppercase" /></td>
                <td className="p-2"><input value={r.cpf} onChange={(e) => update(r.id, 'cpf', e.target.value)} className="w-full border px-2 py-1.5" /></td>
                <td className="p-2"><input value={r.cnhRegistro} onChange={(e) => update(r.id, 'cnhRegistro', e.target.value)} className="w-full border px-2 py-1.5" /></td>
                <td className="p-2"><input value={r.cnhCategoria} onChange={(e) => update(r.id, 'cnhCategoria', e.target.value.replace(/[“”]/g, '').toUpperCase())} className="w-full border px-2 py-1.5 uppercase text-center" /></td>
                <td className="p-2"><input value={r.periodo} onChange={(e) => update(r.id, 'periodo', e.target.value)} className="w-full border px-2 py-1.5" /></td>
                <td className="p-2"><input value={r.cargaHoraria} onChange={(e) => update(r.id, 'cargaHoraria', e.target.value)} className="w-full border px-2 py-1.5" /></td>
                <td className="p-2"><input value={r.dataEmissao} onChange={(e) => update(r.id, 'dataEmissao', e.target.value)} className="w-full border px-2 py-1.5" /></td>
                <td className="p-2"><button onClick={(e) => { e.stopPropagation(); removeRecipient(r.id); }} className="p-2 hover:bg-red-50 text-red-600 cursor-pointer"><Trash2 className="w-4 h-4" /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
